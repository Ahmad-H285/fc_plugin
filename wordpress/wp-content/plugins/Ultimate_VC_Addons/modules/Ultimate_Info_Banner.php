...
if(!class_exists('Ultimate_Info_Banner')) 
{
	class Ultimate_Info_Banner{
		function __construct(){
			...
			add_action('wp_enqueue_scripts', array($this, 'register_info_banner_assets'));
		}
		function register_info_banner_assets()
		{
			wp_register_script('utl-info-banner-script',plugins_url('../assets/js/info-banner.js',__FILE__),array(), ULTIMATE_VERSION);
			wp_register_style('utl-info-banner-style',plugins_url('../assets/css/info-banner.css',__FILE__),array(), ULTIMATE_VERSION);
		}
		...
		function banner_shortcode($atts)
		{
			...
			$output .= '<div id="ultib3-'.$id.'" class="ultb3-box '.$el_class.' '.$ib3_effect.'" style="'.$style.'">';
				...
				if($button_text != '')
					$output .= '<a href="'.$button_link.'" class="ultb3-btn" style="'.$button_style_inline.'">'.$button_text.'<i class="dashicons dashicons-arrow-right-alt2"></i></a>';
				$output .= '</div>';
			$output .= '</div>';
